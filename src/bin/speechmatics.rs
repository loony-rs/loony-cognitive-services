...
use loony_cognitive_services::config::{get_audio_format, get_transcription_config};
use loony_cognitive_services::realtime::ReadMessage;
use loony_cognitive_services::realtime::models::{self, EndOfStream, StartRecognition};
...
async fn main() {
    env_logger::init();
    let api_key = std::env::var("SPEECHMATICS_API_KEY").unwrap();
    ...
    log::info!("Speechmatics listening on {}", url);
...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>) {
    ...
    let (mut speechmatics_sender, mut speechmatics_receiver) =
        connect_speechmatics(state.api_key.clone()).await.unwrap();
                        ...
                        if let Ok(data) = data {
                            match data {
                                ...
                                ReadMessage::AddTranscript(msg) => {
                                    log::info!("AddTranscript: {:#?}", msg.metadata);
                                }
                                _ => {}
                            }
                        }
...
async fn connect_speechmatics(
    api_key: String,
) -> std::result::Result<(SpeechmaticsSender, SpeechmaticsReceiver), ()> {
    let url = format!("wss://eu2.rt.speechmatics.com/v2?jwt={}", api_key);
    let authorization = format!("Bearer {}", api_key);
    let url = Url::parse(&url).unwrap();
    let sec_key: String = rng()
